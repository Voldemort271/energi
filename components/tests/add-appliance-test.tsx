...
const AddApplianceTest = () => {
	const { setFloatingNav } = useAppPreferencesContext();

	const startVariant1Test = () => {
		setFloatingNav(false);
		startTimedTask('add_appliance_bottom_nav');
	};

	const abortVariant1Test = () => {
		abortTimedTask('add_appliance_bottom_nav');
	};

	const startVariant2Test = () => {
		setFloatingNav(true);
		startTimedTask('add_appliance_floating_nav');
	};

	const abortVariant2Test = () => {
		abortTimedTask('add_appliance_floating_nav');
	};
	...
	return (
		<div className="bg-foreground/5 flex w-full flex-col gap-0 rounded-lg px-4 py-5">
			...
			<div className="text-foreground/70 mt-1 w-full text-sm font-normal">
				You will need to add an appliance of your choice in this test.
				You&apos;re welcome to take a moment to familiarise yourself with the
				interface prior to taking the test.
			</div>
			<div className="h-2.5" />
...
export default AddApplianceTest;
